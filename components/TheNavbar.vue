<template>
  <nav class="h-20 border-b-2 border-dark-600 animate-slow animate-fadeInLeft">
    <div class="flex items-center maxcontainer h-full">
      <div
      ...
      <div class="flex items-center space-x-4 lg:(ml-auto space-x-6) text-dark-500 font-medium">
        ...
        <a
          v-if="!loggedIn"
          class="flex items-center space-x-2 hover:(text-orange-400 cursor-pointer)"
        ...
        >
          ...
          <NuxtLink to="/settings">
            <AppDropdownItem>{{ $t('dropdown.settings') }}</AppDropdownItem>
          </NuxtLink>

          <AppDropdownItem @click="logout()">{{ $t('dropdown.logout') }}</AppDropdownItem>
        </AppDropdown>
      </div>
    </div>
  </nav>
